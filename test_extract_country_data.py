...
def test_AEZ_vs_excel():
    df = pd.read_csv('results/AEZ-by-country.csv').set_index('Country')
    df_region = pd.DataFrame(0, index=['OECD90', 'Eastern Europe', 'Asia (Sans Japan)',
        'Middle East and Africa', 'Latin America', 'China', 'India', 'EU', 'USA'],
        columns=df.columns.copy())
    for country, row in df.iterrows():
        ...
        if region is not None:
            df_region.loc[region, :] += row
    for dataset in [gaez_tropical_humid, gaez_temperate_boreal_humid, gaez_tropical_semiarid,
            gaez_temperate_boreal_semiarid, gaez_global_arid, gaez_global_arctic]:
        gaez_region = pd.DataFrame(dataset[1:], columns=dataset[0]).set_index('Region')
        for region in gaez_region.index:
            tested = total_actual = total_expected = 0
            for col in gaez_region.columns:
                if 'AEZ' not in col:
                    continue
                tested += 1
                expected = gaez_region.loc[region, col]
                actual = df_region.loc[region, col]
                total_actual += actual
                total_expected += expected
                if expected < 10000:
                    continue
                print(f"{region} {col}: {actual} <> {expected}")
                assert actual >= expected * 0.0
                assert actual <= expected * 30.0
            assert tested >= 29
